...
from rtf_parser import document
from rtf_parser.format import PythReader
...
class Group(object):
	...
	def handle_cell(self, onOff=None):
		p = Para()
		self.content.append(p)
		self.currentParaTag = p
		val = onOff in (None, "", "1")
		self.content.append(ReadableMarker("cell", val))
	...
	def handle_headerl(self):
		''

	def handle_headerr(self):
		''

	def handle_headerf(self):
		''

	def handle_footer(self):
		''

	def handle_footerl(self):
		''

	def handle_footerr(self):
		''

	def handle_footerf(self):
		''
